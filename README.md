...
Semantic html are where we use elements with specific names to describe what they actually doi=.

2. Name two big differences between ```display: block;``` and ```display: inline;```.

Inline do not start on new lines and are text based, block start new lines and are structural.
...
Allows everyone to work on the same project at the same time and being able to merge all files without conflict.
